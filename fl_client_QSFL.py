    ...
    def register_handles(self):
        ...
        def on_request_update(*args):
            ...
            if cur_round == 0:
                ...
                self.previous_global_model = copy.deepcopy(weights)

            my_weights, train_loss = self.local_model.train_one_round()

            print(train_loss)

            ####### Compute the relevance between the trained local model & the received global model ########
            local_weigths = copy.deepcopy(my_weights)
            compare_global_model = copy.deepcopy(self.previous_global_model)

            X_bak = copy.deepcopy(local_weigths)
            for i in range(len(X_bak)):
                X_bak[i] = X_bak[i].reshape(-1, 1)
            concatenate_X = np.concatenate(X_bak)

            XX_bak = copy.deepcopy(compare_global_model)
            for i in range(len(XX_bak)):
                XX_bak[i] = XX_bak[i].reshape(-1, 1)
            concatenate_XX = np.concatenate(XX_bak)

            sign_X = [np.sign(i).astype(int) for i in concatenate_X]
            sign_XX = [np.sign(i).astype(int) for i in concatenate_XX]

            count = 0
            for i in range(len(sign_X)):
                if sign_X[i] == sign_XX[i]:
                    count += 1

            relevant = count / len(sign_X)
            self.logger.info(f'relevant:{relevant}')


            ######### Model Segmentation & top-p sparsification wihin segment #########
            # flatten model
            X = copy.deepcopy(my_weights)
            for i in range(len(X)):
                X[i] = X[i].reshape(-1, 1)
            concatenate_X = np.concatenate(X)

            # divide segments
            num_seg = 6 # hyperparameter: NS
            num_paras_segment = np.round(len(concatenate_X) / num_seg).astype(int)
            segments = []
            start_point = 0
            for i in range(num_seg):
                segments.append(concatenate_X[start_point:start_point + num_paras_segment])
                start_point = start_point + num_paras_segment

            # choose one segment to be uploaded
            seg_id = (cur_round + self.clientID) % num_seg
            to_compress = segments[seg_id]


            # self.A_local = np.zeros(to_compress.shape)
            # uncompressed_paras = to_compress + self.A_local
            uncompressed_paras = to_compress
            sorted_concatenate_X = np.argsort(np.abs(uncompressed_paras), axis=0).tolist()
            total_counts = len(to_compress)


            p = 0.4  # hyperparameter: top-p
            remained_paras_count = round(p * total_counts)
            remained_paras_index = sorted_concatenate_X[-remained_paras_count::]

            compressed_paras = []
            for i in range(len(remained_paras_index)):
                compressed_paras.append(uncompressed_paras[remained_paras_index[i]].tolist())
            print('compressed_paras:', compressed_paras)


            # compressed_paras_with_zeros = np.zeros(to_compress.shape)
            # for i in range(len(remained_paras_index)):
            #     compressed_paras_with_zeros[remained_paras_index[i]] = compressed_paras[i]
            # # print('compressed_paras_with_zeros:', compressed_paras_with_zeros)
            #
            # self.A_local = self.A_local + uncompressed_paras - compressed_paras_with_zeros
            # # print('updated self.A_local:', self.A_local)


            pickle_string_remained_paras_index = obj_to_pickle_string(remained_paras_index)
            pickle_string_compressed_paras = obj_to_pickle_string(compressed_paras)
            pickle_string_relevant = obj_to_pickle_string(relevant)


            resp = {
                'round_number': cur_round,
                'remained_paras_index': pickle_string_remained_paras_index,
                'compressed_paras': pickle_string_compressed_paras,
                'relevant':pickle_string_relevant,
        ...
        @profile(precision=4,stream=open(f'memory_profiler{clientID}.log','w+'))
        def on_stop_and_eval(*args):
            ...
            if req['weights_format'] == 'pickle':
                weights = pickle_string_to_obj(req['current_weights'])
                self.previous_global_model = copy.deepcopy(weights)
                # self.logger.info(f'------weights shape-------:{[i.shape for i in weights]}')
                self.local_model.set_weights(weights)
            print('get weights')
...
if __name__ == "__main__":
    ...
    try:
        FederatedClient("127.0.0.1", opt.port, opt.config_file, opt.gpu, opt.ignore_load, opt.clientID)
