...
dataset = "street_8" #femnist_3500 , street_12, street_5
model = "yolo"    #femnistcnn, yolo
...
if model == "faster":
    ...
    server_task_config["MIN_NUM_WORKERS"] = num_client
    server_task_config["MAX_NUM_ROUNDS"] = 2000
    server_task_config["NUM_TOLERATE"] = -1
    server_task_config["NUM_CLIENTS_CONTACTED_PER_ROUND"] = num_client
    server_task_config["ROUNDS_BETWEEN_VALIDATIONS"] = 1000
    # server_eval for rp"
    server_task_config["server_eval"] = "data/{}/server_eval/test.txt".format(dataset, )
    with open(server_task_file, "w") as f:
        json.dump(server_task_config, f, indent=4)
...
elif model == "femnistcnn":
    required_num_client = 18*2
    server_task_file = os.path.join("task_configs", model, dataset, model + "_task.json")
    os.makedirs(os.path.dirname(server_task_file), exist_ok=True)
    server_task_config = dict()
    server_task_config["model_name"] = "FEmnistCNN"
    server_task_config["model_config"] = os.path.join("data", "task_configs", model, dataset, "femnistcnn_model.json")
    server_task_config["log_dir"] = "{}/{}".format(model, dataset)
    server_task_config["model_path"] = "femnistcnn_model.pkl"
    server_task_config["MIN_NUM_WORKERS"] = required_num_client
    server_task_config["MAX_NUM_ROUNDS"] =200
    server_task_config["NUM_TOLERATE"] = -1
    server_task_config["NUM_CLIENTS_CONTACTED_PER_ROUND"] = required_num_client
    server_task_config["ROUNDS_BETWEEN_VALIDATIONS"] = 1000
    # server_eval for rp"
    server_task_config["server_eval"] = "data/femnist_3500/server_eval/server_eval.json"
    with open(server_task_file, "w") as f:
        json.dump(server_task_config, f, indent=4)

    model_config_file = os.path.join("task_configs", model, dataset, model + "_model.json")
    model_config = dict()
    model_config["model_def"] = "config/femnistcnn-custom-{}.cfg".format(dataset.split('_')[0])
    # model_config["pretrained_weights"] = "weights/darknet53.conv.74"
    model_config["multiscale_training"] = True
    model_config["gradient_accumulations"] = 2
    # model_config["img_size"] = 416
    ...
    for i in range(1, required_num_client + 1):
        idx = str(i-1)
        idjson = json.load(open('./femnist_3500/idx.json','r+'))
        task_file_path = os.path.join("task_configs", model, dataset, "femnistcnn_task" + str(i) + ".json")
        task_config = dict()
        task_config["model_name"] = "FEmnistCNN"
        task_config["model_config"] = "data/task_configs/{}/{}/femnistcnn_model.json".format(model, dataset)
        task_config["log_filename"] = "{}/{}/FL_client_{}_log".format(model, dataset, str(i))
        task_config["train"] = "data/femnist_3500/train/{}.json".format(idjson[idx])
        task_config["test"] = "data/femnist_3500/test/{}.json".format(idjson[idx])
        # task_config["names"] = "data/{}/classes.names".format(dataset)
        task_config["n_cpu"] = 4
        task_config["local_epoch"] = 10
        task_config["batch_size"] = 2
