...
class GlobalModel(object):
    ...
    def evaluate(self):
        try:
            loss, acc, recall = self.model.evaluate_server()
...
class Aggregator(object):
    ...
    def update_weights(self, remained_paras_index,compressed_paras, clientIDs, relevants, client_sizes,client_losses):

        ###### QJ #######
        client_contributions = np.array(client_losses) / np.abs(client_sizes)
        norm_contributions = client_contributions / sum(client_contributions)

        beta = 0.9 # hyperparameter: \beta
        new_metrics = [(1-beta) * relevants[i] +  beta* norm_contributions[i] for i in range(len(relevants))]
        self.logger.info(f'new_metrics：{new_metrics}')

        client_num = len(remained_paras_index)


        alpha = 0.7 # hyperparameter: \alpha
        alpha_ = (1-alpha) * 100
        Threshold = np.percentile(new_metrics,alpha_)
        for i in range(client_num):
            if new_metrics[i] < Threshold:
                new_metrics[i] = 0

        ###### SCSS #######
        current_round = server.current_round

        # this can be obtained by set NS=1, p=1, and execute this framework in the 0-th round
        pretrained_weights = pickle.load(open('pretrained_weights_cnn', 'rb'))

        segments = defaultdict(list)

        XX = copy.deepcopy(pretrained_weights[0])
        shapes = []
        for i in range(len(XX)):
            shapes.append(XX[i].shape)
            XX[i] = XX[i].reshape(-1, 1)
        concatenate_XX = np.concatenate(XX)

        num_seg = 6
        num_paras_segment = np.round(len(concatenate_XX) / num_seg).astype(int)

        for c in range(len(client_sizes)):
            cur_client_id = clientIDs[c]
            cur_seg_id = (current_round + cur_client_id) % num_seg
            current_segment = np.zeros((num_paras_segment))

            if new_metrics[cur_client_id-1] == 0:
                pass
            else:
                ## decode
                current_remained_paras_index = remained_paras_index[c]
                current_compressed_paras = compressed_paras[c]
                for i in range(len(current_remained_paras_index)):
                    current_segment[current_remained_paras_index[i]] = current_compressed_paras[i]
                    segments[cur_seg_id].append(current_segment)

        ## aggregation within segment
        for seg_id, seg_values in segments.items():
            # self.logger.info(f'seg_id={seg_id}')
            start_index = num_paras_segment * seg_id
            # self.logger.info(f'start_index：{start_index}')
            sum_vec = np.zeros((num_paras_segment))
            # self.logger.info(f'sum_vec：{len(sum_vec)}')
            for item in seg_values:
                sum_vec += item
                # self.logger.info('+++')
            avg_vec = sum_vec / len(seg_values)
            # self.logger.info('Aggregate end')
            concatenate_XX[start_index:start_index + num_paras_segment] = avg_vec.reshape(-1, 1)
        # self.logger.info('segment mean')


        ###### reshape the original model's shape ######
        original_XX = []
        current_element_index_in_XX = 0
        current_sub_matrix = 0

        # print('concatenate_XX.shape[0]:', concatenate_XX.shape[0])
        while current_element_index_in_XX < concatenate_XX.shape[0]:
            current_count = reduce(lambda x, y: x * y, shapes[current_sub_matrix])
            original_XX.append(
                concatenate_XX[current_element_index_in_XX:current_element_index_in_XX + current_count].reshape(
                    shapes[current_sub_matrix]))
            current_sub_matrix = current_sub_matrix + 1
            current_element_index_in_XX += current_count
        self.current_weights = original_XX
    ...
    def aggregate_loss_accuracy_recall(self, client_losses, client_maps, client_recalls, client_sizes, cur_round):
        cur_time = int(round(time.time())) - self.training_start_time
        aggr_loss, aggr_map, aggr_recall = self.aggregate_loss_map_recall(client_losses, client_maps,
                                                                          client_recalls,
                                                                          client_sizes)
...
class FLServer(object):
    ...
    def register_handles(self):
        ...
        def default_handler(e):
            with open('exception.txt', 'a+') as f:
                f.write('\n\n\n##############################################################\n')
                f.write(f'A new Exception Raise When {time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}\n')
                f.write('---------------------------------------------------------------\n')
                f.write(f'str(e): {str(e)}\n')
                f.write(f'repr(e): {repr(e)}\n')
                exc_type, exc_value, exc_traceback = sys.exc_info()
                f.write("Note, object e and exc of Class %s is %s the same.\n" % (type(exc_value), ('not', '')[exc_value is e]))
        ...
        def handle_client_update(data):
            ...
            if data['round_number'] == self.current_round:
                ...
                self.current_round_client_updates[-1]['relevant'] =  pickle_string_to_obj(data['relevant'])
                self.current_round_client_updates[-1]['remained_paras_index'] = pickle_string_to_obj(
                    data['remained_paras_index'])
                self.current_round_client_updates[-1]['compressed_paras'] = pickle_string_to_obj(
                    data['compressed_paras'])
                self.current_round_client_updates[-1]['clientID'] = data['clientID']

                if len(self.current_round_client_updates) == self.NUM_CLIENTS_CONTACTED_PER_ROUND:

                    self.current_round_client_updates.sort(key=lambda x: int(x['clientID']))

                    self.aggregator.update_weights(
                        [x['remained_paras_index'] for x in self.current_round_client_updates],
                        [x['compressed_paras'] for x in self.current_round_client_updates],
                        [x['clientID'] for x in self.current_round_client_updates],
                        [x['relevant'] for x in self.current_round_client_updates],
    ...
    def check_client_resource(self):
        ...
        self.logger.info(f'self.NUM_CLIENTS_CONTACTED_PER_ROUND:{self.NUM_CLIENTS_CONTACTED_PER_ROUND}')
        print('send weights')
